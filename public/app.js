...
function renderActiveOrders(orders) {
    ...
    container.innerHTML = filteredOrders.map((order) => {
        ...
        const isWaiting = lifecycleStatus === 'pending' && !order.otp_code;
        const hasPhone = order.phone_number && order.phone_number !== 'Processing...';
        const serviceLogo = renderServiceLogo(order.service_type, 'md');
        const logoImg = serviceLogo.includes('<img') 
            ? serviceLogo.match(/<img[^>]+>/)?.[0] || '' 
            : `<div style="width:22px;height:22px;display:flex;align-items:center;justify-content:center;font-size:18px;color:#22c55e">${serviceLogo}</div>`;
        
        // Build the card HTML based on state
        let actionSection = '';
        let spinnerSection = '';
        
        if (isOtpReady) {
            // After OTP received: Only yellow COMPLETE ORDER button
            actionSection = `
                <button class="sms-order-btn sms-order-btn--complete" data-action="complete-order" data-order-id="${escapeAttr(order.id)}">
                    COMPLETE ORDER
                </button>
            `;
            // Show OTP code with copy button
            spinnerSection = `
                <div class="sms-order-otp-wrap">
                    <div class="sms-order-otp-code">${escapeHtml(order.otp_code)}</div>
                    <button type="button" class="sms-order-otp-copy" data-action="copy-otp" data-value="${escapeAttr(order.otp_code)}">
                        COPY CODE
                    </button>
                </div>
            `;
        } else if (isWaiting) {
            // Before OTP: Only CANCEL button (green)
            actionSection = `
                <button class="sms-order-btn sms-order-btn--cancel" data-action="cancel-order" data-order-id="${escapeAttr(order.id)}" ${canCancel ? '' : 'disabled'}>
                    CANCEL
                </button>
            `;
            // Spinner below button
            spinnerSection = `
                <div class="sms-order-spinner-wrap">
                    <div class="sms-order-spinner"></div>
                    <div class="sms-order-spinner-text">Code will be here</div>
                </div>
            `;
        }
        
        return `
            <article class="sms-order-card ${isOtpReady ? 'sms-order-card--ready' : ''}">
                <!-- Top Row: Service | Country Flag | Phone + Copy -->
                <div class="sms-order-top">
                    <div class="sms-order-service-row">
                        <div class="sms-order-icon">${logoImg}</div>
                        <div class="sms-order-service-info">
                            <div class="sms-order-service-name">${escapeHtml(meta.label)}</div>
                            <div class="sms-order-country">
                                <span>${escapeHtml(countryFlag)}</span>
                                <span>${escapeHtml(order.country || 'Unknown')}</span>
                            </div>
                        </div>
                    </div>
                    <div class="sms-order-phone-section">
                        <span class="sms-order-phone">${escapeHtml(order.phone_number || '...')}</span>
                        ${hasPhone ? `<button type="button" class="sms-order-copy-btn" data-action="copy-number" data-value="${escapeAttr(order.phone_number)}" title="Copy number"><i class="fa-regular fa-copy"></i></button>` : ''}
                    </div>
                </div>
                
                <!-- Info Grid: STATUS | TIME | PRICE -->
                <div class="sms-order-info-grid">
                    <div class="sms-order-info-cell">
                        <div class="sms-order-info-label">STATUS</div>
                        <div class="sms-order-info-value">
                            <span class="sms-order-status-badge ${isOtpReady ? 'sms-order-status-badge--received' : 'sms-order-status-badge--waiting'}">
                                ${escapeHtml(isOtpReady ? 'SMS was received' : 'Waiting for SMS')}
                            </span>
                        </div>
                    </div>
                    <div class="sms-order-info-cell">
                        <div class="sms-order-info-label">TIME</div>
                        <div class="sms-order-info-value" data-order-expiry-id="${escapeAttr(order.id)}">${escapeHtml(timeStatus)}</div>
                    </div>
                    <div class="sms-order-info-cell">
                        <div class="sms-order-info-label">PRICE</div>
                        <div class="sms-order-info-value">${escapeHtml(formatMoneyPrecise(order.price))}</div>
                    </div>
                </div>
                ...
                ${isWaiting ? `<div class="sms-order-cancel-timer">Cancel available in <span data-cancel-unlock-id="${escapeAttr(order.id)}">${escapeHtml(getCancelUnlockText(order))}</span></div>` : ''}
            </article>
        `;
    }).join('');
    refreshInlineOrderCountdowns();
...
function renderAdminStats(stats) {
    ...
    // Build summary cards for Overall and Today
    const summaryCards = `
        <div class="grid grid-cols-2 gap-4 mb-6">
            <div class="bg-gradient-to-br from-emerald-50 to-emerald-100 border border-emerald-200 rounded-2xl p-4">
                <div class="text-11px font-bold text-emerald-600 uppercase tracking-wider mb-1">Overall Profit</div>
                <div class="text-22px font-black text-emerald-800">${escapeHtml(formatMoneyPrecise(stats.totalProfit))}</div>
                <div class="text-11px text-emerald-600 mt-1">${escapeHtml(formatMoneyPrecise(stats.totalIncome))} income - ${escapeHtml(formatMoneyPrecise(stats.totalApiCost))} cost</div>
            </div>
            <div class="bg-gradient-to-br from-amber-50 to-amber-100 border border-amber-200 rounded-2xl p-4">
                <div class="text-11px font-bold text-amber-600 uppercase tracking-wider mb-1">Today Profit</div>
                <div class="text-22px font-black text-amber-800">${escapeHtml(formatMoneyPrecise(stats.todayProfit))}</div>
                <div class="text-11px text-amber-600 mt-1">${escapeHtml(formatMoneyPrecise(stats.todayIncome))} income - ${escapeHtml(formatMoneyPrecise(stats.todayApiCost))} cost</div>
            </div>
        </div>
    `;
    
    // Build daily history rows
    let historyRows = '';
    if (stats.dailyHistory && stats.dailyHistory.length > 0) {
        historyRows = stats.dailyHistory.map((day, index) => {
            const dateObj = new Date(day.date);
            const dateStr = dateObj.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
            const isToday = isTodayDate(day.date);
            return `
                <tr class="border-b border-slate-100 align-top ${isToday ? 'bg-amber-50/50' : ''} ${index === 0 ? '' : ''}">
                    <td class="px-4 py-3 whitespace-nowrap font-semibold ${isToday ? 'text-amber-700' : 'text-slate-700'}">
                        ${isToday ? '<span class="inline-block mr-1">●</span>' : ''}${escapeHtml(dateStr)}
                    </td>
                    <td class="px-4 py-3 whitespace-nowrap text-slate-600">${escapeHtml(day.orderCount)} orders</td>
                    <td class="px-4 py-3 whitespace-nowrap font-semibold text-slate-900">${escapeHtml(formatMoneyPrecise(day.income))}</td>
                    <td class="px-4 py-3 whitespace-nowrap text-slate-600">${escapeHtml(formatMoneyPrecise(day.apiCost))}</td>
                    <td class="px-4 py-3 whitespace-nowrap font-bold ${day.profit >= 0 ? 'text-emerald-600' : 'text-rose-600'}">${escapeHtml(formatMoneyPrecise(day.profit))}</td>
                </tr>
            `;
        }).join('');
    } else {
        historyRows = `
            <tr>
                <td colspan="5" class="px-4 py-8 text-center text-slate-500">
                    No profit history available yet. Complete orders to see daily earnings.
                </td>
            </tr>
        `;
    }
    
    // Summary table rows for Overall/Today
    const summaryRows = `
        <tr class="border-b border-slate-200 align-top bg-slate-50">
            <td class="px-4 py-3 whitespace-nowrap font-bold text-slate-900">Overall</td>
            <td class="px-4 py-3 whitespace-nowrap font-semibold text-slate-900">-</td>
            ...
            <td class="px-4 py-3 whitespace-nowrap font-bold text-emerald-700">${escapeHtml(formatMoneyPrecise(stats.totalProfit))}</td>
        </tr>
        <tr class="border-b border-slate-200 align-top bg-amber-50">
            <td class="px-4 py-3 whitespace-nowrap font-bold text-amber-700">Today</td>
            <td class="px-4 py-3 whitespace-nowrap font-semibold text-slate-900">-</td>
            ...
            <td class="px-4 py-3 whitespace-nowrap font-bold text-emerald-700">${escapeHtml(formatMoneyPrecise(stats.todayProfit))}</td>
        </tr>
    `;
    ...
    container.innerHTML = `
        ${summaryCards}
        <div class="mb-4">
            <h4 class="text-sm font-bold text-slate-800 mb-2">Daily Profit History (Last 30 Days)</h4>
        </div>
        ${renderAdminTable(
            ['Date', 'Orders', 'Income', 'API Cost', 'Profit'],
...
async function loadAdminData() {
    ...
    try {
        ...
        // Calculate daily profit stats for history
        const dailyStats = {};
        successfulOrders.forEach((order) => {
            const timestamp = getOrderFinancialTimestamp(order);
            if (!timestamp) return;
            const date = new Date(timestamp);
            const dateKey = date.toISOString().split('T')[0];
            const profit = getOrderProfit(order);
            if (!dailyStats[dateKey]) {
                dailyStats[dateKey] = { date: dateKey, profit: 0, orderCount: 0, income: 0, apiCost: 0 };
            }
            dailyStats[dateKey].profit += profit;
            dailyStats[dateKey].orderCount += 1;
            dailyStats[dateKey].income += Number(order.price || 0);
            dailyStats[dateKey].apiCost += Number(order.provider_cost_pkr || 0);
        });
        const dailyHistory = Object.values(dailyStats)
            .sort((a, b) => new Date(b.date) - new Date(a.date))
            .slice(0, 30);
        
        renderAdminStats({ totalDeposits, todayDeposits, totalIncome, todayIncome, totalApiCost, todayApiCost, totalProfit, todayProfit, dailyHistory });
...
function updateProcessingModalState() {
    ...
    const subtitle = qs('processing-subtitle');
    const list = qs('processing-active-list');

    // Update subtitle with queue count
    if (subtitle) {
        subtitle.textContent = requestCount === 1
            ? '1 number in queue'
    ...
    // Update queue list
    if (list) {
        list.innerHTML = activeRequests.slice(0, 5).map((entry) => `
            <div class="processing-queue-row">
                <span class="processing-queue-spinner" aria-hidden="true"></span>
                <div class="processing-queue-info">
                    <div class="processing-queue-service">${escapeHtml(getServiceMeta(entry.serviceType).label)} ${getCountryFlagByName(entry.countryName)}</div>
                    <div class="processing-queue-meta">${escapeHtml(entry.countryName)}</div>
                </div>
                <span class="processing-queue-price">${escapeHtml(formatMoney(entry.price))}</span>
                <button type="button" class="processing-queue-cancel" data-action="cancel-single-purchase" data-request-id="${escapeAttr(entry.requestId)}" title="Cancel this request">✕</button>
            </div>
        `).join('');
    }
    ...
    const shell = qs('processing-shell');
    if (shell) shell.classList.remove('processing-shell--minimized');
    if (!document.querySelector('.app-modal:not(.hidden):not(#processing-modal)')) {
        document.body.classList.remove('overflow-hidden');
    }
}

// Toggle minimize/expand for processing modal
function toggleProcessingModal() {
    const shell = qs('processing-shell');
    if (shell) {
        shell.classList.toggle('processing-shell--minimized');
    }
}
...
function bindStaticEvents() {
    ...
    qs('processing-toggle-btn')?.addEventListener('click', toggleProcessingModal);
    ...
    document.addEventListener('click', async (event) => {
        ...
        if (action === 'toggle-processing-modal') {
            toggleProcessingModal();
            return;
        }
        if (action === 'cancel-single-purchase') {
            const requestId = actionTarget.dataset.requestId;
            if (requestId && state.purchaseRequests.has(requestId)) {
                const requestMeta = state.purchaseRequests.get(requestId);
                requestMeta.mutedAbortToast = true;
                try {
                    requestMeta.controller.abort();
                } catch {
