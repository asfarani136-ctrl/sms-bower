...
function renderDailyProfit(successfulOrders) {
    const container = qs('admin-daily-profit-list');
    if (!container) return;
    if (!successfulOrders.length) {
        container.innerHTML = renderEmptyState('No profit data yet', 'Daily profit will appear here once orders are completed.');
        return;
    }
    const dailyMap = {};
    successfulOrders.forEach((order) => {
        const ts = getOrderFinancialTimestamp(order);
        if (!ts) return;
        const dateKey = new Date(ts).toISOString().slice(0, 10);
        if (!dailyMap[dateKey]) dailyMap[dateKey] = { income: 0, apiCost: 0, profit: 0 };
        dailyMap[dateKey].income += Number(order.price || 0);
        dailyMap[dateKey].apiCost += Number(order.provider_cost_pkr || 0);
        dailyMap[dateKey].profit += getOrderProfit(order);
    });
    const sortedDates = Object.keys(dailyMap).sort((a, b) => b.localeCompare(a));
    let overallIncome = 0, overallApiCost = 0, overallProfit = 0;
    const rows = sortedDates.map((dateKey) => {
        const d = dailyMap[dateKey];
        overallIncome += d.income;
        overallApiCost += d.apiCost;
        overallProfit += d.profit;
        const displayDate = new Date(dateKey + 'T00:00:00').toLocaleDateString('en-PK', { year: 'numeric', month: 'short', day: 'numeric' });
        const isToday = isTodayDate(dateKey + 'T12:00:00');
        return `
            <tr class="border-b border-slate-200 align-top last:border-b-0 ${isToday ? 'bg-emerald-50/40' : ''}">
                <td class="px-4 py-3 whitespace-nowrap font-semibold ${isToday ? 'text-emerald-700' : 'text-slate-900'}">${escapeHtml(displayDate)}${isToday ? ' (Today)' : ''}</td>
                <td class="px-4 py-3 whitespace-nowrap font-semibold text-slate-900">${escapeHtml(formatMoneyPrecise(d.income))}</td>
                <td class="px-4 py-3 whitespace-nowrap text-slate-600">${escapeHtml(formatMoneyPrecise(d.apiCost))}</td>
                <td class="px-4 py-3 whitespace-nowrap font-semibold ${d.profit >= 0 ? 'text-emerald-600' : 'text-rose-600'}">${escapeHtml(formatMoneyPrecise(d.profit))}</td>
            </tr>`;
    }).join('');
    const totalRow = `
        <tr class="border-t-2 border-slate-300 align-top bg-slate-50/60">
            <td class="px-4 py-3 whitespace-nowrap font-bold text-emerald-700">Overall Total</td>
            <td class="px-4 py-3 whitespace-nowrap font-bold text-slate-900">${escapeHtml(formatMoneyPrecise(overallIncome))}</td>
            <td class="px-4 py-3 whitespace-nowrap font-bold text-slate-600">${escapeHtml(formatMoneyPrecise(overallApiCost))}</td>
            <td class="px-4 py-3 whitespace-nowrap font-bold ${overallProfit >= 0 ? 'text-emerald-700' : 'text-rose-700'}">${escapeHtml(formatMoneyPrecise(overallProfit))}</td>
        </tr>`;
    container.innerHTML = renderAdminTable(
        ['Date', 'Income', 'API Cost', 'Real Profit'],
        rows + totalRow,
        'min-w-[640px]'
    );
}
...
async function loadAdminData() {
    ...
    try {
        ...
        renderDailyProfit(successfulOrders);
