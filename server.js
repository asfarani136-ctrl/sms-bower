...
const MAX_OPEN_ORDERS_PER_SERVICE_COUNTRY = 10;
...
async function initDB() {
    ...
    await queryRun(`
        ...
        WHERE o.id = r.id
          AND r.rn > ${MAX_OPEN_ORDERS_PER_SERVICE_COUNTRY}
    `);
    await queryRun('DROP INDEX IF EXISTS idx_orders_open_service_country_unique');
    await queryRun("CREATE INDEX IF NOT EXISTS idx_orders_open_service_country_lookup ON orders (user_id, service_type, country_id) WHERE COALESCE(status, order_status, 'pending') IN ('pending', 'active')");
...
function normalizeServiceLookupKey(value) {
    return String(value || '')
        .toLowerCase()
        .normalize('NFKD')
        .replace(/[\u0300-\u036f]/g, '')
...
function createGeneratedServiceType(label, code = '') {
    const normalizedLabel = normalizeServiceLookupKey(label).replace(/\s+/g, '');
    if (normalizedLabel) return normalizedLabel;
    return String(code || '').toLowerCase().replace(/[^a-z0-9]+/g, '').trim();
}

function createGeneratedIndonesiaCountry() {
    return { name: 'Indonesia', code: '+62', price: 100, countryId: 6, flag: '🇮🇩' };
}

function buildExtraServiceCatalogEntries() {
    try {
        const catalogPath = path.join(__dirname, 'public', 'All Social Media Platform.txt');
        ...
        const seenLabels = new Set(
            Object.values(serviceCatalog).map((serviceConfig) => {
                return normalizeServiceLookupKey(String(serviceConfig.serviceName || '').replace(/\s+Number$/i, ''));
            }).filter(Boolean)
        );
        const extras = {};
        for (let index = 0; index < tokens.length - 1; index += 1) {
            ...
            const normalizedLabel = normalizeServiceLookupKey(label);
            if (!normalizedLabel || seenLabels.has(normalizedLabel)) {
                if (tokens[index + 2] === label) index += 2;
                continue;
            }
            const serviceType = createGeneratedServiceType(label, code);
            if (!serviceType || serviceCatalog[serviceType] || extras[serviceType]) {
                if (tokens[index + 2] === label) index += 2;
                continue;
            }
            ...
            extras[serviceType] = {
                serviceType,
                serviceName: `${label} Number`,
                serviceCode: code,
                countries: [createGeneratedIndonesiaCountry()]
            };
...
Object.assign(serviceCatalog, buildExtraServiceCatalogEntries());
...
app.post('/api/order', ensureAuth, async (req, res) => {
    ...
    try {
        ...
        if (Number(activeSameServiceCountry.rows[0]?.total || 0) >= MAX_OPEN_ORDERS_PER_SERVICE_COUNTRY) {
            await client.query('ROLLBACK');
            return res.status(409).send(`You can only keep ${MAX_OPEN_ORDERS_PER_SERVICE_COUNTRY} active orders for this service and country at the same time.`);
        }
